...
UPLOAD_FOLDER = r'C:\Users\sujat\OneDrive\Desktop\Automatic_attendance_system\Face-Recognition-Attendance-System-main\IMAGE_FILES'
...
def create_csv_file():
    ...
    # Check if the CSV file already exists, create it if necessary
        file_path = os.path.join(directory, file_name)
        if not os.path.exists(file_path):
            with open(file_path, 'w') as f:
                f.write("Roll No,Date,Time,Status\n")
    
    # Add your additional logic if needed
    
    # You can return the file path if you want to use it elsewhere in the code
        return file_path
...
def gen():
    ...
    # IMAGEtest_FILES = []
    filename = []
    dir_path = r'C:\Users\sujat\OneDrive\Desktop\Automatic_attendance_system\Face-Recognition-Attendance-System-main\IMAGE_FILES'
    ...
    def takeAttendence(roll_no):
            with open(csv_file, 'r+') as f:
                ...
                if roll_no not in roll_List:
                    now = datetime.now()
                    datestring = now.strftime('%H:%M:%S')
                    status='Present'
                    today=now.date()
                    f.writelines(f'\n{roll_no},{today},{datestring},{status}' )
                    frequency=1000
                    duration= 1000
